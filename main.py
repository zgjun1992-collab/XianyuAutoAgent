    ...
    async def handle_message(self, message_data, websocket, send_ack=True):
        ...
        try:
            ...
            if conversation_state in {"aftersale", "aftersale_pending"}:
                # Migrate old versions that locked every later message into a
                # fixed receipt. The structured query context now carries the
                # refund stage while ordinary product questions remain usable.
                resume = getattr(self.app_store, "resume_conversation", None)
                if resume:
                    resume(scope_id)
