...
const appIdentity = reactive({ edition: 'V3.6', frontend_version: '', backend_version: '', build_commit: '' })
...
async function refresh(quiet = true) {
  // Never pile up multi-megabyte snapshots on slower customer databases.
...
async function refreshServiceStatus() {
  const state = await call('GET', '/service/status', undefined, true)
...
async function chooseExcel() {
  try {
    ...
    if (file) {
      ...
      if (!importDraft.name) importDraft.name = file.split(/[\\/]/).pop().replace(/^\d+-[a-f0-9]{8}-/, '').replace(/\.[^.]+$/, '')
    }
  } catch (error) {
    const raw = error.message || String(error)
    notify(raw.replace(/^Error invoking remote method '[^']+':\s*Error:\s*/i, '').replace(/^Error:\s*/i, ''), 'error')
  }
}
...
onMounted(async () => {
  // Render build identity even when one local-backend startup request fails.
  // Promise.all used to discard these successful results after any rejection.
  const [identityResult, updateResult] = await Promise.allSettled([desktop.getVersion(), desktop.getUpdateState()])
  if (identityResult.status === 'fulfilled') Object.assign(appIdentity, identityResult.value || {})
  if (updateResult.status === 'fulfilled') Object.assign(updater, updateResult.value || {})
  await Promise.allSettled([refresh(true), getConfig(), refreshLicense(true)])
  ...
  refreshTimer = setInterval(() => refreshServiceStatus().catch(() => {}), 2500)
  nextTick(syncBrowserBounds)
})
        ...
        <div class="editor-panel scroll-panel">
          ...
          <div v-else-if="productTab === 'stores'" class="product-tab-body">
            ...
            <article class="knowledge-card sku-store-card">
              ...
              <p v-else>当前商品同步数据中没有SKU。请先同步商品；若闲鱼商品本身没有多规格，则无需单独设置规格门店。</p>
            </article>
          </div>
