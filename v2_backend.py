...
class BackendState:
    ...
    def _summary_source_payload(self, product):
        ...
        payload = {
            "source_priority": ["canonical_skus", "effective_knowledge", "page_description"],
            "product_identity": {
    ...
    def _normalize_summary_structure(structured, canonical_skus):
        ...
        by_key = {str(row.get("sku_key") or ""): row for row in profiles if isinstance(row, dict) and row.get("sku_key")}
        by_name = {
        ...
        for sku in canonical_skus:
            ...
            profile = dict(by_key.get(str(sku.get("sku_key") or "")) or by_name.get(re.sub(r"\s+", "", name).lower()) or {})
            profile.update({
                "sku_key": sku.get("sku_key") or "", "sku_name": name, "name": name,
                "option_type": sku.get("option_type") or "", "sale_price": sku.get("sale_price") or "",
                "face_value": sku.get("face_value") or "", "composition": sku.get("composition") or "",
                "stock": sku.get("stock") or "", "sellable": bool(sku.get("sellable", True)),
            })
            inherited = []
            ...
            option = {
                ...
                "sale_price": profile["sale_price"], "face_value": profile["face_value"],
                "composition": profile["composition"], "stock": profile["stock"],
                "sellable": profile["sellable"], "applicable_time": profile.get("applicable_time") or "",
