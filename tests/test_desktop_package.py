...
class DesktopPackageTests(unittest.TestCase):
    ...
    def test_auto_update_channel_is_configured(self):
        package = json.loads((DESKTOP / "package.json").read_text(encoding="utf-8"))
        self.assertEqual("0.11.31", package["version"])
        self.assertIn("electron-updater", package["dependencies"])
        self.assertEqual(
            [{"provider": "generic", "url": "https://download.yituan123.com/v3.6"}],
            package["build"]["publish"],
        )
        self.assertIn("合并为一条完整回复", package["build"]["releaseInfo"]["releaseNotes"])
        self.assertIn("严格使用真实SKU名称", package["build"]["releaseInfo"]["releaseNotes"])
        self.assertIn("不再把叠加规则自动拼进SKU名称", package["build"]["releaseInfo"]["releaseNotes"])
